...
import { fetchAllCustomers, fetchAllItems, fetchAllInvoices, upsertCustomers, upsertItems, upsertInvoices, getCompanyInfo, } from './qbo.js';
import { mountQboRoutes } from './qboRoutes.js';
import { pool } from './db.js';
import keepaliveRoute from './qboKeepaliveRoute.js';
const app = express();
...
async function tableExists(name) {
    const { rows } = await pool.query(`SELECT (to_regclass($1) IS NOT NULL) AS exists`, [name]);
...
app.get('/api/integrations', async (_req, res) => {
    try {
        if (await tableExists('public.integrations')) {
            const { rows } = await pool.query(`SELECT provider, org_id, updated_at
           FROM integrations
          WHERE provider = $1 AND org_id = $2
          ORDER BY updated_at DESC`, [INTEGRATION_PROVIDER, INTEGRATION_ORG_ID]);
            if (rows.length) {
...
app.get('/api/db/health', async (_req, res) => {
    try {
        // reuse your exported pool
        const { pool } = await import('./db.js'); // NOTE: .js extension on ESM import
...
app.get('/api/dashboard/stats', async (_req, res) => {
    try {
        if (await tableExists('public.invoices')) {
            const { rows } = await pool.query(`WITH last30 AS (
           SELECT *
             FROM invoices
...
app.get('/api/customers', async (_req, res) => {
    try {
        if (await tableExists('public.customers')) {
            const { rows } = await pool.query(`SELECT id, name, email, created_at
           FROM customers
...
app.get('/api/products', async (_req, res) => {
    try {
        if (await tableExists('public.products')) {
            const { rows } = await pool.query(`SELECT id, name, sku, unit_price_cents, updated_at
           FROM products
...
app.get('/api/invoices', async (_req, res) => {
    try {
        if (await tableExists('public.invoices')) {
            const { rows } = await pool.query(`SELECT id, customer_name, total_cents, status, issued_at, due_at
           FROM invoices
...
async function getLatestQboTokenOfficial(integrationId) {
    const { rows } = await pool.query(`SELECT access_token, refresh_token, expires_at, realm_id, updated_at
       FROM qbo_tokens
...
app.get('/api/qbo/status', async (_req, res) => {
    try {
        const { rows } = await pool.query(`SELECT (to_regclass('public.integrations') IS NOT NULL) AS has_integrations,
        ...
        if (!hasTokens)
            return res.status(404).json({ hasToken: false });
        if (hasIntegrations) {
