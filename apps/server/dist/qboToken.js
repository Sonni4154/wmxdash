...
let refreshing = Promise.resolve(undefined);
let busy = false;
...
async function loadToken() {
    const rows = await query(`SELECT provider, realm_id, access_token, refresh_token, expires_at
                                      FROM qbo_tokens ORDER BY updated_at DESC LIMIT 1`);
    return rows[0] || null;
}
async function saveToken(t) {
    await query(`INSERT INTO qbo_tokens (provider, realm_id, access_token, refresh_token, expires_at, updated_at)
     VALUES ($1,$2,$3,$4,$5,now())
     ON CONFLICT (realm_id)
     DO UPDATE SET access_token=EXCLUDED.access_token,
                   refresh_token=EXCLUDED.refresh_token,
                   expires_at=EXCLUDED.expires_at,
                   updated_at=now()`, [t.provider, t.realm_id, t.access_token, t.refresh_token, t.expires_at]);
}
export async function refreshAccessTokenIfNeeded(minSecondsLeft = 300) {
    if (busy) {
    ...
    if (secondsLeft(row.expires_at) > minSecondsLeft)
        return; // still good
    // lock
    busy = true;
    refreshing = (async () => {
